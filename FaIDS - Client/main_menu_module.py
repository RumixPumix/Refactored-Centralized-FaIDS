...
from server_communication_helper_func import get_current_file_transfer_ready_users, send_file_to_user, send_request_to_user, receive_request_from_user, receive_file_from_user, send_to_server
from logging_module import log

MAIN_MENU_OPTIONS = ["Send A File", "Receive A File", "Sub-Domain Request"]

def file_sending_menu(socket, username): #Add a refresh option for current users, also add possibility to send it to multiple users!
    selected_file = list_options_func(os.listdir("files/send"))
    if selected_file is None:
        log("No files are placed inside /files/send folder!", 2)
        return
    selected_user = list_options_func(get_current_file_transfer_ready_users(socket, username)) #DODAT OPCIJU "R" KAO REFRESH NA USER LIST.
    ...
    request_response = send_request_to_user(socket, username, selected_file, selected_user)
    if request_response == True:
        log(f"Recipient {selected_user} accepted, sending file...", 3)
        response = send_file_to_user(socket, username, selected_file, selected_user)
        if response == None:
...
def file_receiving_menu(socket, username):
    print("Waiting for incoming file...")
    file_request_message = receive_request_from_user(socket, username)
    ...
    if response == "Accept":
        ...
        response = receive_file_from_user(socket)
        if response == None:
        ...
        if send_to_server(socket,2,3,None):
...
def main_menu(socket, username, configuration):
    while True:
        main_menu_index = 1
        ...
        try:
            ...
            match selected_option:
                case 0:
                    file_sending_menu(socket, username)
                case 1:
                    file_receiving_menu(socket, username)
                case 3:
                    sub_domain_request_menu()
        except ValueError:
            main_menu(socket, username, configuration)
